...
def push_module_to_new_repo(child_module_path, sid):
    """Push the generated Terraform module to a new Git repository."""
    try:
        # Generate a unique repository name
        repo_name = f"terraform-module-{int(time.time())}"
        
        # Retrieve GitHub token from environment variable
        github_token = os.environ.get('GITHUB_TOKEN')
        if not github_token:
            error_message = "GitHub token not found in environment variables."
            print(error_message)
            socketio.emit('log', error_message, to=sid)
            return None
        ...
        if response.status_code == 201:
            ...
            # Initialize Git and push the module
            subprocess.run(['git', 'init'], cwd=child_module_path)
            subprocess.run(['git', 'remote', 'add', 'origin', repo_clone_url], cwd=child_module_path)
            subprocess.run(['git', 'add', '.'], cwd=child_module_path)
            subprocess.run(['git', 'commit', '-m', 'Add generated Terraform module'], cwd=child_module_path)
            subprocess.run(['git', 'push', '-u', 'origin', 'master'], cwd=child_module_path)
            # Emit the repository URL to the frontend via Socket.IO
            socketio.emit('repo_url', repo_info['html_url'], to=sid)
            return repo_info['html_url']
        else:
            error_message = f"Failed to create repository: {response.text}"
            print(error_message)
            socketio.emit('log', error_message, to=sid)
            return None
    except Exception as e:
        error_message = f"Error pushing module to repository: {str(e)}"
        print(error_message)
        socketio.emit('log', error_message, to=sid)
        return None
...
def run_script():
    ...
    try:
        ...
        # Push the generated module to a new repository
        message = "Pushing generated module to a new repository..."
        print(message)
        socketio.emit('log', message, to=sid)
        repo_url = push_module_to_new_repo(child_module, sid)
        if not repo_url:
            error_message = "Failed to push module to repository."
            print(error_message)
            socketio.emit('log', error_message, to=sid)

        # Return the import log, results, and repo URL in the response
        return jsonify({
